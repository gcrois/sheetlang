...
fn main() {
    println!("cargo:rerun-if-env-changed=SHEETLANG_DISABLE_DEMOS");
    ...
    // Generate demo scripts code (can be disabled for troubleshooting)
    let disable_demos = env::var("SHEETLANG_DISABLE_DEMOS")
        .ok()
        .map(|v| v != "0" && v.to_lowercase() != "false")
        .unwrap_or(false);

    if disable_demos {
        generate_empty_demo_scripts();
    } else {
...
fn generate_demo_scripts() {
    ...
    for (index, entry) in entries.iter().enumerate() {
        ...
        let content = fs::read_to_string(&path)
            .unwrap_or_else(|e| panic!("Failed to read script {}: {}", filename_str, e));
    ...
    // Generate code to get demo filename
    code.push_str("pub fn get_demo_name(n: u8) -> Option<&'static str> {\n");
    code.push_str("    match n {\n");
...
fn generate_empty_demo_scripts() {
    let out_dir = env::var("OUT_DIR").unwrap();
    let dest_path = Path::new(&out_dir).join("demo_scripts.rs");
    let mut code = String::new();
    code.push_str("// Auto-generated by build.rs (demos disabled)\n\n");
    code.push_str("pub fn get_demo_script(_n: u8) -> Option<&'static str> {\n");
    code.push_str("    None\n");
    code.push_str("}\n\n");
    code.push_str("pub fn get_demo_name(_n: u8) -> Option<&'static str> {\n");
    code.push_str("    None\n");
    code.push_str("}\n\n");
    code.push_str("pub fn get_demo_count() -> u8 {\n");
    code.push_str("    0\n");
    code.push_str("}\n");
    fs::write(&dest_path, code).unwrap();
}
